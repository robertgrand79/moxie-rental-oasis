-- Drop the existing function first (parameter names don't match)
DROP FUNCTION IF EXISTS public.copy_organization_demo_data(uuid, uuid, uuid, jsonb);

-- Recreate with correct properties table columns
-- The old function referenced columns like 'name', 'slug' that don't exist
-- Properties table uses 'title' instead of 'name' and has different columns
...
BEGIN
  ...
  -- Uses actual column names from properties table
  -- ==========================================
  ...
  IF COALESCE((_demo_config->>'testimonials')::boolean, (_demo_config->>'copy_testimonials')::boolean, true) THEN
    INSERT INTO testimonials (
      id, organization_id, guest_name, property_id, content, rating,
      stay_date, source, is_featured, is_verified, display_order,
      is_active, created_at, updated_at
    )
    SELECT 
      gen_random_uuid(), _target_org_id, guest_name,
      CASE 
        WHEN property_id IS NOT NULL AND _property_id_map ? property_id::text 
        THEN (_property_id_map->>property_id::text)::uuid 
        ELSE NULL 
      END,
      content, rating, stay_date, source, is_featured, is_verified,
      display_order, is_active, now(), now()
    FROM testimonials
    WHERE organization_id = _source_org_id;
