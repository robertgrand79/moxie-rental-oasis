...
-- Fix existing org owners who have staff team_role
UPDATE organization_members 
SET team_role = 'owner' 
WHERE role = 'owner' AND (team_role IS NULL OR team_role = 'staff');

-- Fix create_organization_with_owner to set team_role = 'owner'
CREATE OR REPLACE FUNCTION public.create_organization_with_owner(_name text, _slug text, _user_id uuid, _template_id uuid DEFAULT NULL::uuid, _visual_template_id uuid DEFAULT NULL::uuid, _include_demo_data boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
