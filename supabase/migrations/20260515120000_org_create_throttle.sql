-- Throttle self-serve organization creation to prevent abuse.
-- A verified user can still spam this RPC to create dozens of empty orgs.
-- Cap at 3 orgs per user per rolling 24h window. Platform admins are exempt.
...
BEGIN
  -- Per-user throttle: 3 orgs / 24h. Platform admins (who legitimately
  -- spin up tenant orgs in bulk) are exempt.
  SELECT public.is_platform_admin() INTO _is_platform_admin;

  IF NOT COALESCE(_is_platform_admin, false) THEN
    ...
    IF _recent_count >= 3 THEN
      RAISE EXCEPTION 'You have created the maximum of 3 organizations in the last 24 hours. Please wait before creating another, or contact support if you need a higher limit.'
        USING ERRCODE = 'P0001';
    END IF;
  END IF;
  ...
  IF _visual_template_id IS NOT NULL THEN
    ...
    IF _should_include_demo AND _source_org_id IS NOT NULL THEN
      IF NOT EXISTS (SELECT 1 FROM organizations WHERE id = _source_org_id) THEN
        ...
        VALUES ('warn', 'Template source organization not found, skipping demo data',
          jsonb_build_object('visual_template_id', _visual_template_id, 'source_org_id', _source_org_id, 'template_name', _template_name),
  ...
  INSERT INTO organization_members (organization_id, user_id, role, team_role)
  VALUES (_new_org_id, _user_id, 'owner', 'owner');
  ...
  VALUES (_new_org_id, 'branding'), (_new_org_id, 'contact'), (_new_org_id, 'property'), (_new_org_id, 'payments');

  IF _source_org_id IS NOT NULL THEN
    FOR _setting IN SELECT key, value FROM site_settings WHERE organization_id = _source_org_id
    LOOP
    ...
    INSERT INTO message_templates (organization_id, name, subject, content, category, is_active, created_by)
    SELECT _new_org_id, name, subject, content, category, is_active, _user_id
    FROM message_templates WHERE organization_id = _source_org_id AND is_active = true
    ON CONFLICT DO NOTHING;
  END IF;

  IF _should_include_demo = true AND _source_org_id IS NOT NULL THEN
    _demo_result := copy_organization_demo_data(_source_org_id, _new_org_id, _user_id, COALESCE(_demo_config, '{}'::jsonb));
  END IF;
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM site_settings WHERE organization_id = _new_org_id AND site_settings.key = defaults.key
  )
  ON CONFLICT (organization_id, key) DO NOTHING;

  INSERT INTO application_logs (level, message, context, tags)
  VALUES ('info', 'Organization created from template',
    jsonb_build_object('organization_id', _new_org_id, 'organization_name', _name,
      'visual_template_id', _visual_template_id, 'template_name', _template_name,
      'source_org_id', _source_org_id, 'pricing_tier_id', _pricing_tier_id,
      'pricing_tier_name', _pricing_tier_name, 'include_demo_data', _should_include_demo,
