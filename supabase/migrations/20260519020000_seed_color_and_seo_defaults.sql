-- Seed earth-tone color defaults and basic SEO metadata for fresh signups.
--
-- Background: a fresh tenant created without a template currently gets:
--   - no color tokens (site renders with raw Tailwind defaults)
--   - no SEO metadata (social shares and search snippets are blank)
--
-- The Starter template advertises a "color_scheme: earth_tones" intent but
-- the seed never wrote color keys. This adds:
--   - colorPrimary/Secondary/Accent matching the Starter template's existing
--     palette so the seed and template are consistent
--   - metaDescription, ogTitle, ogDescription derived from the org name so
--     social shares and search snippets are populated from day one
--
-- Hero background image and favicon are intentionally left empty here;
-- they need image-source decisions tracked separately.
