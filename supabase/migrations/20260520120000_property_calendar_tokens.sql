-- Move calendar export tokens out of the publicly-readable `properties` table.
--
-- `properties` has a public SELECT policy (qual = true), so any anonymous
-- visitor could read every row's `calendar_export_token` and pull any
-- property's iCal availability feed. Tokens now live in a dedicated,
-- RLS-protected table that anon cannot read.

CREATE TABLE IF NOT EXISTS public.property_calendar_tokens (
  property_id uuid PRIMARY KEY REFERENCES public.properties(id) ON DELETE CASCADE,
  calendar_export_token text NOT NULL DEFAULT encode(extensions.gen_random_bytes(16), 'hex'),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.property_calendar_tokens ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON public.property_calendar_tokens FROM anon;
GRANT SELECT ON public.property_calendar_tokens TO authenticated;
GRANT ALL ON public.property_calendar_tokens TO service_role;

-- Only users who can manage the property may read its token. No anon access.
DROP POLICY IF EXISTS "Managers can view calendar tokens" ON public.property_calendar_tokens;
CREATE POLICY "Managers can view calendar tokens"
  ON public.property_calendar_tokens
  FOR SELECT
  TO authenticated
  USING (public.can_manage_property(auth.uid(), property_id));

-- Backfill existing tokens, preserving values so iCal subscription URLs
-- already distributed to Airbnb/VRBO keep working.
INSERT INTO public.property_calendar_tokens (property_id, calendar_export_token)
SELECT id, calendar_export_token
FROM public.properties
WHERE calendar_export_token IS NOT NULL
ON CONFLICT (property_id) DO NOTHING;

-- Every new property automatically gets a token row.
CREATE OR REPLACE FUNCTION public.create_property_calendar_token()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  INSERT INTO public.property_calendar_tokens (property_id)
  VALUES (NEW.id)
  ON CONFLICT (property_id) DO NOTHING;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_create_property_calendar_token ON public.properties;
CREATE TRIGGER trg_create_property_calendar_token
  AFTER INSERT ON public.properties
  FOR EACH ROW
  EXECUTE FUNCTION public.create_property_calendar_token();

-- Point the regenerate RPC at the new table.
...
BEGIN
  ...
  INSERT INTO public.property_calendar_tokens (property_id, calendar_export_token, updated_at)
  VALUES (p_property_id, new_token, now())
  ON CONFLICT (property_id)
  DO UPDATE SET calendar_export_token = EXCLUDED.calendar_export_token, updated_at = now();

  RETURN new_token;
END;
$$;

-- Stop the org-clone routine from writing tokens into `properties`; the
-- AFTER INSERT trigger above now provisions them in property_calendar_tokens.
  ...
  IF COALESCE((_demo_config->>'properties')::boolean, (_demo_config->>'copy_properties')::boolean, true) THEN
    ...
    LOOP
      ...
      INSERT INTO properties (
        ...
        pricelabs_listing_id, latitude, longitude,
