-- Fix user deletion failures caused by tables in the public schema referencing public.profiles without CASCADE or SET NULL
-- Any foreign key constraint pointing to public.profiles with NO ACTION ('a') or RESTRICT ('r') is changed to ON DELETE SET NULL.
-- We also drop NOT NULL on those columns to allow SET NULL.
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format(
      'ALTER TABLE %s ADD CONSTRAINT %I FOREIGN KEY (%s) REFERENCES public.profiles(%s) ON DELETE SET NULL',
      r.table_name,
      r.conname,
      r.cols_list,
      r.ref_cols_list
