      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className="h-8 rounded-full border-border/30"
          >
            <Star className={cn('h-3.5 w-3.5', thread.status === 'starred' && 'fill-current')} strokeWidth={1.5} />
          </Button>
          <Button
            variant={thread.status === 'resolved' ? 'default' : 'outline'}
            size="sm"
            onClick={() => onStatusChange(thread.status === 'resolved' ? 'active' : 'resolved')}
            className="h-8 rounded-full gap-1.5 border-border/30"
          >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-8 w-8 p-0 rounded-full border-border/30">
            ...
            <DropdownMenuContent align="end">
              {[1, 4, 24, 72].map(h => (
                <DropdownMenuItem key={h} onClick={() => onSnooze(addHours(new Date(), h))}>
                  <AlarmClock className="h-4 w-4 mr-2" strokeWidth={1.5} />
          ...
          {isSnoozed && (
            <Badge variant="outline" className="text-xs ml-auto rounded-full border-border/30">
              <AlarmClock className="h-3 w-3 mr-1" strokeWidth={1.5} />
        ...
        {/* --- Guest Profile --- */}
        <section>
          <h4 className="text-[10px] font-medium text-muted-foreground/60 uppercase tracking-widest mb-3">
            Guest
          </h4>
          <div className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-muted/10 p-4 space-y-3">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-primary/5 flex items-center justify-center shrink-0">
                <User className="h-5 w-5 text-primary" strokeWidth={1.5} />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-medium tracking-tight truncate">{thread.guest_name || 'Unknown Guest'}</p>
              </div>
            </div>
          ...
          {loadingReservations ? (
            ...
            <div className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-muted/10 p-4 space-y-3">
              <div className="flex items-start gap-2.5">
                <Home className="h-4 w-4 text-muted-foreground/60 mt-0.5 shrink-0" strokeWidth={1.5} />
                <span className="text-sm font-medium tracking-tight leading-tight">
                  {activeReservation.property?.title || 'Unknown Property'}
                </span>
              </div>
              <div className="flex items-center gap-2.5">
                <Calendar className="h-4 w-4 text-muted-foreground/60 shrink-0" strokeWidth={1.5} />
              ...
              {reservations.length > 1 && (
                <div className="pt-3 border-t border-border/20">
                  <p className="text-[10px] text-muted-foreground/50 uppercase tracking-widest mb-2">
                    {reservations.length - 1} more reservation{reservations.length > 2 ? 's' : ''}
                  </p>
                  {reservations.filter(r => r.id !== activeReservation.id).map(res => (
                    <div key={res.id} className="text-xs text-muted-foreground py-1.5">
            ...
            <div className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-muted/10 p-6 text-center">
              <FileText className="h-6 w-6 mx-auto mb-1.5 text-muted-foreground/30" strokeWidth={1.5} />
              <p className="text-xs text-muted-foreground/50">No reservations found</p>
            </div>
          )}
        </section>
        ...
        {/* --- Notes --- */}
        <section>
          <h4 className="text-[10px] font-medium text-muted-foreground/60 uppercase tracking-widest mb-3 flex items-center gap-1.5">
            <StickyNote className="h-3.5 w-3.5" strokeWidth={1.5} />
            Notes
          </h4>
          <div className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-muted/10 p-4 text-sm text-muted-foreground/50 italic">
            No notes yet
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-[10px] font-medium text-muted-foreground/60 uppercase tracking-widest flex items-center gap-1.5">
          ...
          <div className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-muted/10 p-4">
            {aiSummary ? (
          ...
          {/* Stats */}
          <div className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-muted/10 p-4 mt-3 space-y-2.5">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground/60 flex items-center gap-1.5">
                <MessageSquare className="h-3.5 w-3.5" strokeWidth={1.5} />
                Messages
              </span>
              <span className="font-medium tabular-nums text-foreground">{messages.length}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground/60">Inbound</span>
              <span className="font-medium tabular-nums text-foreground">{messages.filter(m => m.direction === 'inbound').length}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground/60">Outbound</span>
              <span className="font-medium tabular-nums text-foreground">{messages.filter(m => m.direction === 'outbound').length}</span>
            </div>
          </div>
        </section>
      </div>
